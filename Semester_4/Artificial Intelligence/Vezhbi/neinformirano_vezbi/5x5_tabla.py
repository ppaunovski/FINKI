...
def move_right(state, i):
    squares = list(state)
    square_to_move = squares[i]
    new_square = (square_to_move[0]+1, square_to_move[1])
    squares[i] = new_square
    return tuple(squares)


def move_left(state, i):
    squares = list(state)
    square_to_move = squares[i]
    new_square = (square_to_move[0] - 1, square_to_move[1])
    squares[i] = new_square
...
def move_up(state, i):
    squares = list(state)
    square_to_move = squares[i]
    new_square = (square_to_move[0], square_to_move[1]+1)
    squares[i] = new_square
    return tuple(squares)
...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(5):
            new_state = move_up(state, i)
            if Squares.check_valid(new_state):
                succ[f'Pomesti kvadratche {i + 1} gore'] = new_state
            new_state = move_down(state, i)
            if Squares.check_valid(new_state):
                succ[f'Pomesti kvadratche {i + 1} dolu'] = new_state
            new_state = move_right(state, i)
            if Squares.check_valid(new_state):
                succ[f'Pomesti kvadratche {i+1} desno'] = new_state
            new_state = move_left(state, i)
            if Squares.check_valid(new_state):
                succ[f'Pomesti kvadratche {i + 1} levo'] = new_state
