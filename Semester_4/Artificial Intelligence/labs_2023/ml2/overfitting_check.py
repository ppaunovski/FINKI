...
def get_train_and_test():
    train = get_dataset()[:int(0.8*len(get_dataset()))]
    test = get_dataset()[int(0.8*len(get_dataset())):]
...
if __name__ == '__main__':
    ...
    clf = MLPClassifier(neurons, max_iter=20, learning_rate_init=learning_rate, activation='relu', random_state=0)

    column = int(input())
    test_case = [float(elem) for elem in input().split()]

    train, test = get_train_and_test()

    train_x, train_y = divide_x_y(train)
    test_x, test_y = divide_x_y(test)

    scaler.fit(train_x)

    clf.fit(scaler.transform(train_x), train_y)

    predictions_test = clf.predict(scaler.transform(test_x))
    predictions_train = clf.predict(scaler.transform(train_x))
    ...
    if acc_train <= 1.15 * acc_test:
        ...
        print(clf.predict(scaler.transform([test_case]))[0])
    else:
        print("Se sluchuva overfitting")
        train_x = np.delete(train_x, column, axis=1)
        test_x = np.delete(test_x, column, axis=1)
        test_case = np.delete([test_case], column, axis=1)

        clf_2 = MLPClassifier(neurons, max_iter=20, learning_rate_init=learning_rate, activation='relu', random_state=0)

        scaler_2 = MinMaxScaler(feature_range=(-1, 1))
        scaler_2.fit(train_x)

        clf_2.fit(scaler_2.transform(train_x), train_y)

        print(clf_2.predict(scaler_2.transform(test_case))[0])
