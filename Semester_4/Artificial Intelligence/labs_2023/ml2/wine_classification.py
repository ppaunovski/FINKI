...
def first_x_from_all_classes_for_training(x1, x2):
    c_0 = [row for row in get_dataset() if row[-1] == 0]
    c_1 = [row for row in get_dataset() if row[-1] == 1]
    c_2 = [row for row in get_dataset() if row[-1] == 2]

    return c_0[int(x1 * len(c_0)):int(x2 * len(c_0))] + c_1[int(x1 * len(c_1)):int(x2 * len(c_1))] + c_2[int(x1 * len(c_2)):int(x2 * len(c_2))]


def first_x_from_all_classes_for_test(x1, x2):
    c_0 = [row for row in get_dataset() if row[-1] == 0]
    c_1 = [row for row in get_dataset() if row[-1] == 1]
    c_2 = [row for row in get_dataset() if row[-1] == 2]

    return c_0[int(x2 * len(c_0)):] + c_1[int(x2 * len(c_1)):] + c_2[int(x2 * len(c_2)):]
...
if __name__ == '__main__':
    ...
    nb_train = class1[:int(x1*len(class1))] + class2[:int(x1*len(class2))] + class3[:int(x1*len(class3))]
    dt_train = class1[int(x1*len(class1)):int(x2*len(class1))] + class2[int(x1*len(class2)):int(x2*len(class2))] + class3[int(x1*len(class3)):int(x2*len(class3))]
    rf_train = class1[:int(x2*len(class1))] + class2[:int(x2*len(class2))] + class3[:int(x2*len(class3))]

    test = class1[int(x2*len(class1)):] + class2[int(x2*len(class2)):] + class3[int(x2*len(class3)):]

    # nb_train = first_x_from_all_classes_for_training(0, x1)
    # dt_train = first_x_from_all_classes_for_training(x1, x2)
    # rf_train = first_x_from_all_classes_for_training(0, x2)
    #
    # test = first_x_from_all_classes_for_test(x1, x2)

    x, y = divide_x_y(nb_train)
    gauss.fit(x, y)

    x, y = divide_x_y(rf_train)
    random_forest.fit(x, y)

    x, y = divide_x_y(dt_train)
    d_tree.fit(x, y)

    x, y = divide_x_y(test)

    pred_nb = gauss.predict(x)
    pred_dt = d_tree.predict(x)
    pred_rf = random_forest.predict(x)
    acc = 0

    for prediction_nb, prediction_dt, prediction_rf, correct in zip(pred_nb, pred_dt, pred_rf, y):
        votes = {0:0, 1:0, 2:0}

        votes[prediction_nb] += 1
        votes[prediction_dt] += 1
        votes[prediction_rf] += 1

        print(votes[0], votes[1], votes[2], correct)

        if votes[0] >= 2 and 0 == correct:
            acc += 1
        elif votes[1] >= 2 and 1 == correct:
            acc += 1
        elif votes[2] >= 2 and 2 == correct:
            acc += 1

    print(f"Tocnost: {acc/len(test)}")
