...
if __name__ == '__main__':
    ...
    test_case = [test_case[i] for i in range(len(test_case)) if i != col_index]

    random_forest = RandomForestClassifier(criterion=crit, random_state=0, n_estimators=num_trees)
    ...
    train_set = new_dataset[:int(0.85 * len(new_dataset))]
    test_set = new_dataset[int(0.85 * len(new_dataset)):]

    train_set_attributes = [row[:-1] for row in train_set]
    train_set_class = [row[-1] for row in train_set]

    test_set_attributes = [row[:-1] for row in test_set]
    test_set_class = [row[-1] for row in test_set]

    random_forest.fit(train_set_attributes, train_set_class)
    hit_count = 0

    for test, correct in zip(test_set_attributes, test_set_class):
        guess = random_forest.predict([test])
        if guess == correct:
            hit_count += 1

    print(f'Accuracy: {hit_count/len(test_set)}')
    print(random_forest.predict([test_case])[0])
    print(random_forest.predict_proba([test_case])[0])