...
def divide_dataset(dataset):
    return [row[:-1] + [grade(row[-1])] for row in dataset]
...
if __name__ == '__main__':
    ...
    dataset = divide_dataset(get_dataset())

    # print(dataset)

    train = dataset[:int(x*len(dataset))]
    test = dataset[int(x*len(dataset)):]
    ...
    features = list(dt_classifier.feature_importances_)
    ...
    for row in dataset:
        new_row = []
        for elem in row:
            if row.index(elem) != least_important:
                new_row.append(elem)
        new_dataset.append(new_row)

    new_train = new_dataset[:int(x * len(new_dataset))]
    new_test = new_dataset[int(x * len(new_dataset)):]

    new_train_x = [row[:-1] for row in new_train]
    new_train_y = [row[-1] for row in new_train]

    new_test_x = [row[:-1] for row in new_test]
    new_test_y = [row[-1] for row in new_test]

    mlp_clf1 = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    mlp_clf2 = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    ...
    mlp_clf1.fit(scaler1.transform(new_train_x), new_train_y)
    mlp_clf2.fit(scaler2.transform(new_train_x), new_train_y)

    acc1 = 0
    acc2 = 0

    for guess, correct in zip(new_test_x, new_test_y):
        prediction = mlp_clf1.predict(scaler1.transform([guess]))
        if prediction == correct:
            acc1 += 1

    for guess, correct in zip(new_test_x, new_test_y):
        prediction = mlp_clf2.predict(scaler2.transform([guess]))
        if prediction == correct:
            acc2 += 1

    print(f'Tocnost so StandardScaler: {acc1/len(new_test)}')
    print(f'Tocnost so MinMaxScaler: {acc2/len(new_test)}')
