...
def go_up(state):
    ...
    new_snake_pos = list(snake_position)

    new_snake_pos.append((head[0], head[1] + 1, 'up'))

    return tuple(new_snake_pos), state[1]
...
def go_down(state):
    ...
    new_snake_pos.append((head[0], head[1] - 1, 'down'))
...
def go_right(state):
    ...
    new_snake_pos.append((head[0] + 1, head[1], 'right'))
...
def go_left(state):
    ...
    new_snake_pos.append((head[0] - 1, head[1], 'left'))
...
def valid(state):
    snake_position = state[0]
    head = snake_position[-1]
    if 0 > head[0] or head[0] >= 10 or 0 > head[1] or head[1] >= 10:
        return False
...
def ate_apple(state):
    ...
    new_snake_pos = list(snake_position)
    head = snake_position[-1]
    tail = snake_position[0]

    if (head[0], head[1]) in apples:
        new_apples.remove((head[0], head[1]))
    else:
        new_snake_pos.remove(tail)

    return tuple(new_snake_pos), tuple(new_apples)
...
def move_forward(state):
    ...
    if direction == 'right':
        return go_right(state)

    if direction == 'left':
        return go_left(state)
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        new_state = move_forward(state)
        if valid(new_state):
            new_state = ate_apple(new_state)
    ...
    def h(self, node):
        ...
        # closest_point = (0, 0)
        # for apple in apples:
        #     dist = math.sqrt((head[0] - apple[0])**2 + (head[1] - apple[1])**2)
        #     if dist < min_dist:
        #         min_dist = dist
        #         closest_point = apple
        #
        # return abs(head[0] - closest_point[0]) + abs(head[1] - closest_point[1])

if __name__ == "__main__":
    n = int(input())

    green = [tuple(map(int, input().split(','))) for _ in range(n)]

    snake_pos = ((0, 9, 'down'), (0, 8, 'down'), (0, 7, 'down'))

    snake = Snake((snake_pos, tuple(green)))
