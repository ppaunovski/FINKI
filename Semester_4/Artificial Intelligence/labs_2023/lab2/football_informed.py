...
class Football(Problem):
        ...
        if 0 <= player_x <= 7 and 0 <= player_y + 1 <= 5:
            if player_x == ball_x and player_y + 1 == ball_y:
                if 0 <= ball_x <= 7 and 0 <= ball_y + 1 <= 5 and (ball_x, ball_y + 1) not in unavailable_spots_for_ball:
                    ...
                    successors['Turni topka gore'] = ((player_x, player_y + 1), (ball_x, ball_y + 1), enemies)
            else:
                if (player_x, player_y + 1) != enemy1 and (player_x, player_y + 1) != enemy2:
                    successors['Pomesti coveche gore'] = ((player_x, player_y + 1), (ball_x, ball_y), enemies)
        # move down
        if 0 <= player_x <= 7 and 0 <= player_y - 1 <= 5:
            if player_x == ball_x and player_y - 1 == ball_y:
                if 0 <= ball_x <= 7 and 0 <= ball_y - 1 <= 5 and (ball_x, ball_y - 1) not in unavailable_spots_for_ball:
                    # the ball should move up
                    successors['Turni topka dolu'] = ((player_x, player_y - 1), (ball_x, ball_y - 1), enemies)
            else:
                if (player_x, player_y - 1) != enemy1 and (player_x, player_y - 1) != enemy2:
                    successors['Pomesti coveche dolu'] = ((player_x, player_y - 1), (ball_x, ball_y), enemies)
        # move right
        if 0 <= player_x + 1 <= 7 and 0 <= player_y <= 5:
            if player_x + 1 == ball_x and player_y == ball_y:
                if 0 <= ball_x + 1 <= 7 and 0 <= ball_y <= 5 and (ball_x + 1, ball_y) not in unavailable_spots_for_ball:
                    # the ball should move up
                    successors['Turni topka desno'] = ((player_x + 1, player_y), (ball_x + 1, ball_y), enemies)
            else:
                if (player_x + 1, player_y) != enemy1 and (player_x + 1, player_y) != enemy2:
                    successors['Pomesti coveche desno'] = ((player_x + 1, player_y), (ball_x, ball_y), enemies)
        # move up-right
        if 0 <= player_x + 1 <= 7 and 0 <= player_y + 1 <= 5:
            if player_x + 1 == ball_x and player_y + 1 == ball_y:
                if 0 <= ball_x + 1 <= 7 and 0 <= ball_y + 1 <= 5 and (
                ball_x + 1, ball_y + 1) not in unavailable_spots_for_ball:
                    # the ball should move up
                    successors['Turni topka gore-desno'] = ((player_x + 1, player_y + 1), (ball_x + 1, ball_y + 1), enemies)
            else:
                if (player_x + 1, player_y + 1) != enemy1 and (player_x + 1, player_y + 1) != enemy2:
                    successors['Pomesti coveche gore-desno'] = ((player_x + 1, player_y + 1), (ball_x, ball_y), enemies)
        # move down-right
        if 0 <= player_x + 1 <= 7 and 0 <= player_y - 1 <= 5:
            if player_x + 1 == ball_x and player_y - 1 == ball_y:
                if 0 <= ball_x + 1 <= 7 and 0 <= ball_y - 1 <= 5 and (
                ball_x + 1, ball_y - 1) not in unavailable_spots_for_ball:
                    # the ball should move up
                    successors['Turni topka dolu-desno'] = ((player_x + 1, player_y - 1), (ball_x + 1, ball_y - 1), enemies)
            else:
                if (player_x + 1, player_y - 1) != enemy1 and (player_x + 1, player_y - 1) != enemy2:
                    successors['Pomesti coveche dolu-desno'] = ((player_x + 1, player_y - 1), (ball_x, ball_y), enemies)
        return successors
    ...
    def h(self, node):
        ...
        return min(max(dx1, dy1), max(dx2, dy2))
...
if __name__ == '__main__':
    ...
    football = Football((man_pos, ball_pos, tuple(opponents)), ((7, 2), (7, 3)))
