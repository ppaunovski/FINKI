...
if __name__ == '__main__':
    ...
    train_attributes = [row[:-1] for row in train_set]
    train_class = [row[-1] for row in train_set]

    test_attributes = [row[:-1] for row in test_set]
    test_class = [row[-1] for row in test_set]

    # encoder.fit([row[:-1] for row in dataset])

    # train_attributes = encoder.transform(train_attributes)
    # test_attributes = encoder.transform(test_attributes)

    # print(train_attributes)
    # print(train_class)

    gauss.fit(train_attributes, train_class)

    prediction_accuracy = 0

    for prediction, true_val in zip(test_attributes, test_class):
        # print(gauss.predict([prediction]))
        if gauss.predict([prediction])[0] == true_val:
            prediction_accuracy += 1

    print(prediction_accuracy/len(test_set))

    input_test_case = [float(elem) for elem in input().split(" ")]

    print(int(gauss.predict([input_test_case])[0]))
    print(gauss.predict_proba([input_test_case]))
