package Kolokviumki2.angliskiRecnik;
...
public class Speluvanje {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<N; i++){
            ...
            tabela.insert(new Zbor(word.toLowerCase()), word);
        }

        String sentence = br.readLine();
        String words[] = sentence.split(" ");
        String []incorrect = new String[words.length];
        int index = 0;
        for(int i=0; i<words.length; i++){
            ...
            if(tabela.search(new Zbor(w.toLowerCase())) == -1 && !w.equals("")){
                incorrect[index++] = w;
            }
        }
        if(index == 0){
