...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        ...
        SLLNode<Card> max = new SLLNode<Card>(tmp1.element, null);

        while(tmp1 != null){
            if(max.element.damage() < tmp1.element.damage()){
                max = tmp1;
            }
            tmp1 = tmp1.succ;
        }
        ...
        int i=0;
        while(i < secondFriendCards.size()/2){
            tmp2 = tmp2.succ;
            i++;
        }
        secondFriendCards.insertBefore(max.element, tmp2);
    }
