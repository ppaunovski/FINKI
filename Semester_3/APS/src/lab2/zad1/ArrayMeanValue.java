...
public class ArrayMeanValue {
    ...
    public static int brojDoProsek(Array<Integer> arr) {
        ...
        double avg = sum / arr.getSize();
        int closest = 0;
        if(arr.find((int) avg) != -1) {
            closest = arr.find((int)avg);
            return arr.get(closest);
        }

        double smallestDifferance = Math.abs(arr.get(0) - avg);

        for(int i=1; i<arr.getSize(); i++){

            if(Math.abs(arr.get(i) - avg) < smallestDifferance ||
               Math.abs(arr.get(i) - avg) == smallestDifferance && arr.get(closest) > arr.get(i)){
                smallestDifferance = Math.abs(arr.get(i) - avg);
                closest = i;
            }
        }
        return arr.get(closest);
