...
class Graph<E> {
    ...
    public int getIndex(E president) {
        for (GraphNode<E> eGraphNode : adjList) {
            if(eGraphNode.getInfo().equals(president))
...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        Graph<String> graph = new Graph<>(n, members);
        for(int i=0; i<m; i++){
            String line = br.readLine();
            String []parts = line.split("\\s+");
            int x = Integer.parseInt(parts[0]);
            String xName = parts[1];
            int y = Integer.parseInt(parts[2]);
            String yName = parts[3];
            graph.adjList[x].setInfo(xName);
            graph.adjList[y].setInfo(yName);

            graph.addEdge(x, y, Integer.parseInt(parts[4]));
        }
        ...
        List<Edge> prim = graph.prim(graph.getIndex(president));

        System.out.println(prim.stream().mapToDouble(Edge::getWeight).sum());
    }
