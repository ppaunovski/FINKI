...
class Graph<E> {
    ...
    int getIndex(String name){
        for (GraphNode<E> eGraphNode : adjList) {
            if(eGraphNode.getInfo().equals(name)){
    ...
    void topological_sort_dfs(String last){
        boolean visited[] = new boolean[num_nodes];
        for(int i=0;i<num_nodes;i++){
            visited[i] = false;
        }
        ...
        while(!s.isEmpty()){
            ...
            if(adjList[s.pop()].getInfo().equals(last)){
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<m; i++){
            ...
            for(int j=1; j<parts.length; j++){
                ...
                graph.addEdge(graph.getIndex(parts[j]), graph.getIndex(parts[0]));
            }
        }
        String last = br.readLine();
        br.close();
        graph.topological_sort_dfs(last);
    }
}