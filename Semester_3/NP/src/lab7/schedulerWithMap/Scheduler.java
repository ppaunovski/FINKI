package lab7.schedulerWithMap;

import java.util.*;
import java.util.stream.Collectors;

public class Scheduler<T> {
    private Map<Date, T> eventByDate;

    public Scheduler(){
        eventByDate = new TreeMap<>(Comparator.comparing(Date::getTime));
    }

    void add(Date d, T t){
        eventByDate.putIfAbsent(d, t);
    }

    boolean remove(Date d){
        return eventByDate.remove(d, eventByDate.get(d));
    }

    T next(){
        Date now = new Date();
        Date nextDate = new Date(now.getTime()+18000000);
        Iterator<Date> iterator = eventByDate.keySet().iterator();
        for(int i=0; i<eventByDate.keySet().size(); i++){
            Date next = iterator.next();
            if(next.getTime() > now.getTime() && next.getTime() < nextDate.getTime()){
                nextDate = next;
            }
        }
        return eventByDate.get(nextDate);
    }

    T last(){
        Date now = new Date();
        Date lastDate = new Date(now.getTime()-18000000);
        Iterator<Date> iterator = eventByDate.keySet().iterator();
        for(int i=0; i<eventByDate.keySet().size(); i++){
            Date last = iterator.next();
            if(last.getTime() < now.getTime() && last.getTime() > lastDate.getTime()){
                lastDate = last;
            }
        }
        return eventByDate.get(lastDate);
    }

    ArrayList<T> getAll (Date begin, Date end){
        return (ArrayList<T>) eventByDate.entrySet().stream()
                .filter(dateTEntry -> dateTEntry.getKey().getTime() > begin.getTime() && dateTEntry.getKey().getTime() < end.getTime())
                .map(Map.Entry::getValue)
                .collect(Collectors.toList());
    }

    T getFirst(){
        Date first = eventByDate.keySet().stream().collect(Collectors.toList()).get(0);
        return eventByDate.get(first);
    }
    T getLast(){
        Date first = eventByDate.keySet().stream().collect(Collectors.toList()).get(eventByDate.size()-1);
        return eventByDate.get(first);
    }
}
