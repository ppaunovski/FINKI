...
class File implements Comparable<File>{
    ...
    public String getMonthDay(){
        return String.format("%s-%d", getMonth(), getDay());
    }
...
//                .collect(Collectors.toList());
    ...
    public int totalSizeOfFilesFromFolders(List<Character> folders){
        ...
        for(Character folder : folders){
            if(filesByFolder.containsKey(folder)){
                sum += filesByFolder.get(folder).stream()
    ...
    public Map<Integer, Set<File>> byYear(){
        return filesByFolder.values().stream().flatMap(value -> value.stream()).collect(Collectors.groupingBy(File::getYear, Collectors.mapping(file -> file, Collectors.toSet())));
    }

    public Map<String, Long> sizeByMonthAndDay(){
        return filesByFolder.values().stream().flatMap(value -> value.stream())
                .collect(Collectors.groupingBy(
                        file -> file.getMonthDay(),
